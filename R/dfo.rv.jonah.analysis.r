...
dfo.rv.jonah.analysis <- function(DS='stratified.estimates', out.dir = 'bio.lobster', p=p, ip=NULL,save=T) {
    loc = file.path( project.datadirectory(out.dir), "jonah.analysis" )
...
pi='base'
        ...
        if(p$length.based){
              ...
              if(any(!is.finite(dp$fwt))) {
                  ...
                  fit = nls(fwt~a*flen^b,de[which(de$spec==2511 & is.finite(de$fwt)),],start=list(a=0.001,b=3.3))
                  ab = coef(fit)
