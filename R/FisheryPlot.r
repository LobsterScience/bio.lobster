...
FisheryPlot <- function(data,lfa=NULL,fd=file.path(project.figuredirectory('bio.lobster','ReferencePoints')),title = paste('LFA',lfa),fn=paste0('FisheryPlot',lfa),preliminary=NULL,...) {
	...
	par(mar=c(5.1, 4.1, 4.1, 5.1),las=1)
		...
		if(!is.null(preliminary))lines(data$YEAR[preliminary],data$LANDINGS[preliminary],type='h',pch=21,col=rgb(1,0.6,0),lwd=10,lend=3)
		par(new=T)
