...
CatchRatePlot <- function(data,usr=NULL,lrp=NULL,lfa=NULL,fd=file.path(project.figuredirectory('bio.lobster','ReferencePoints')),title = paste('LFA',lfa),fn=paste0('CatchRateRefs',lfa),...) {
	
			dir.create( fd, recursive = TRUE, showWarnings = FALSE )
					
					ylim = c(0,max(c(data[,2],lrp,usr)))

					plot(data[,1],data[,2],xlab='Year',ylab='CPUE',type='b',lwd=2,pch=16,main=title,ylim=ylim,...)
