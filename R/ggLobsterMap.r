...
ggLobsterMap <- function(
  ...
  # ----------------------
  ns_coast <- readRDS(
    file.path(layerDir,"CoastSF.rds")
  )
  
  grids <- st_make_valid(
    readRDS(file.path(layerDir,"Grids_updated_2026.rds"))
  )
  
  fills=NA
  
  if(addGridsDepthPruned){
    grids <- st_make_valid(
      readRDS(
        file.path(
          layerDir,
  ...
  lfa <- readRDS(
    file.path(layerDir,"LFAPolys37Split.rds")
  )
  
  bath <- readRDS(
    file.path(layerDir,"bathy10-300SF.rds")
  )
  
  nafo_sf <- readRDS(
    file.path(layerDir,"NAFO_sf.rds")
  )
  
  cents <- readRDS(
    file.path(layerDir,"LFALabelsSF.rds")
  )
  ...
  ns_coast <- suppressWarnings(
    st_crop(ns_coast, crop_box)
  )
  
  grids <- suppressWarnings(
    st_crop(grids, crop_box)
  )
  
  lfa <- suppressWarnings(
    st_crop(lfa, crop_box)
  )
  
  bath <- suppressWarnings(
    st_crop(bath, crop_box)
  )
  
  cents <- suppressWarnings(
    st_crop(cents, crop_box)
  )
  
  gridCent <- suppressWarnings(
    st_crop(gridCent, crop_box)
  )
  ...
  if(!is.null(colourLFA)){
    ...
    #
    # Vector:
    #   colourLFA = c("33","34","35")
    #
    #   Each LFA gets a different colour.
    #
    # List:
    #   colourLFA = list(c("33","34"),c("35","36"))
    #
    #   LFAs within each group share a colour.
    #
    if(is.character(colourLFA)){
      ...
      stop(
        "colourLFA must be a character vector or a list"
      )
    }
    ...
    if(length(groups) > length(lfa_colours)){
      ...
      stop(
        "Not enough default colours for number of LFA groups"
      )
    }
  ...
  # Highlight grids
  # ----------------------
  if(!is.null(colourGrid)){
    
    # Normalize colourGrid to a list of grid groups
    #
    # Vector:
    #   colourGrid = c(3301,3302,3303)
    #
    #   Each grid gets a different colour.
    #
    # List:
    #   colourGrid = list(c(3301,3302),c(3303,3304))
    #
    #   Grids within each group share a colour.
    #
    if(is.numeric(colourGrid) || is.character(colourGrid)){
      
      groups <- as.list(colourGrid)
      
    } else if(is.list(colourGrid)){
      ...
      stop(
        "colourGrid must be a numeric/character vector or a list"
      )
    }
    ...
    if(length(groups) > length(lfa_colours)){
      ...
      stop(
        "Not enough default colours for number of grid groups"
      )
    }
    ...
    for(i in seq_along(groups)){
      ...
      grids_to_colour <- as.character(
        groups[[i]]
      )
      
      hl <- subset(
        grids,
        as.character(GRID_NO) %in% grids_to_colour
      )
  ...
  if(addLFALabels){
    ...
    xy <- cbind(
      st_coordinates(cents),
      st_drop_geometry(cents)
    )
