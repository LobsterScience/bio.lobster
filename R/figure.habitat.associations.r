figure.habitat.associations <- function(data,p,out.dir='bio.lobster',plot=T) {
	...
	for(i in 1:(length(data))) {
			...
			if(nchar(survey.dat$mission[1])==10) yr= as.numeric(unique(substr(as.character(survey.dat)$mission,4,7)))
			if(nchar(as.character(survey.dat$mission[1]))==6) yr= as.numeric(unique(substr(as.character(survey.dat$mission),1,4)))
			
if(sum(survey.dat$totno[which(!is.na(survey.dat$bottom.temperature))])>0) {
		...
		if(sum(survey.dat$totno[which(!is.na(survey.dat$bottom.salinity))])>0) {
	...
	if(sum(survey.dat$totno[which(!is.na(survey.dat$z))])>0) {
...
out.d$VAR = 'DEPTH'
	...
	par(mar=c(4,4,0,1))
		...
		with(a3,plot(VAL~YEAR,type='n',ylab=unique(VAR),xlim=c(min(a3$YEAR),max(a3$YEAR)), ylim=yll, xlab='Year'))
		with(pps,polygon(x,y,col= "#C1C1C1",border=F))
		with(a3,lines(VAL~YEAR,lty=1,lwd=2))
		with(a3,lines(MED.50~YEAR,lwd=0.8,col='red'))
		with(a3,points(VAL~YEAR,pch=SYM, cex=1.5))
