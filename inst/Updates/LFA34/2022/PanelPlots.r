...
x1 = data.frame(yr=wMM$yr,wtm=wMM$x,rf = rMM$x)
...
hcrPlot(B=Sp$wtm/1000,mF=Sp$rf,USR=RP[[1]][1]/1000,LRP=RP[[1]][2]/1000,RR=RRs[[1]],labels=c(),yrs=c(rep('',length(Sp$yr)-1),2022),ylim=c(0.5,1.1),area.cols=rev(gray.colors(3,start=.4,end=1)),xlab=' ',ylab='NEFSC Spring',yr.ends=T); box();
hcrPlot(B=Fa$wMM/1000,mF=Fa$rMM,USR=RP[[2]][1]/1000,LRP=RP[[2]][2]/1000,RR=RRs[[2]],labels=c(),yrs=c(rep('',nrow(Fa)-2),2021,""),area.cols=rev(gray.colors(3,start=.4,end=1)),xlab=' ',ylab='NEFSC Fall',yr.ends=F);box();
hcrPlot(B=DF$wMM/1000,mF=DF$rMM,USR=RP[[3]][1]/1000,LRP=RP[[3]][2]/1000,RR=RRs[[3]],labels=c(),yrs=c(rep('',length(DF$yr)-1),2022),ylim=c(0.5,1.1),area.cols=rev(gray.colors(3,start=.4,end=1)),xlab=' ',ylab='DFO RV Survey',yr.ends=T);box();
hcrPlot(B=IL$wMM/1000,mF=IL$rMM,USR=RP[[4]][1]/1000,LRP=RP[[4]][2]/1000,RR=RRs[[4]],labels=c(),yrs=c(rep('',length(IL$yr)-1),2022),ylim=c(0.5,1.1),area.cols=rev(gray.colors(3,start=.4,end=1)),xlab=' ',ylab='ILTS',yr.ends=T);box();
