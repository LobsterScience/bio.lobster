...
require(sf)
la()
fd=file.path(project.datadirectory('Update_LFA41_24'),'outputs','SURVEYS')
setwd(fd)
crs_utm20 <- 32620
...
polys = sf::read_sf(find.bio.gis('BTS_Strata'),crs=4326) ## Convert DataFrame to SF object
polys = st_make_valid(polys) ## Ensure geometry is valid
