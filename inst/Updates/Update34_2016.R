...
# Plot Landings (1892-present) Figure 2
		pdf(file.path( project.figuredirectory("lobster"), "LFA34Landings.pdf"),8,6)
			...
			lines(1976:2015,Rmed,col='green',lwd=3,lty=2)
			axis(side=1,at=round(seq(min(Annual34$YR),p$current.assessment.year-1,length.out=7),0))
			abline(h=USR,col='blue',lwd=2)
			with(Rm, lines(YR, Run.Mean, col='red',lwd=2,lty=1))

			legend('topleft',lwd=3,col=c('black','grey','blue','red','green'),c('Annual','Seasonal','Upper Stock Reference','3-year running mean','3-year running median'),bty='n',cex=0.8)
