...
b = subset(b,SYEAR %in% 2005:2024 & LFA =='38') ### CHECK LFA
...
#effort
ef = merge(cc,aaa)
ef$Effort = ef$LFA38/(ef$CPUE) ###CHECK LFA
...
g1 <- ggplot(data = subset(aae,yr>2004), aes(x = yr,y=LFA38)) + #CHECK LFA
...
g1a <- ggplot(data = ccf, aes(x = yr,y = CPUE)) +
  ...
  labs(x = "Year", y = "Unstandardized CPUE") +
  theme_csas()
...
PR_2024<-PR_2024[c("YEARMTH", "L38PERCENT")]
