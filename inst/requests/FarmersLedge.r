...
require(bio.utilities)
 ...
 LobsterMap('38',label='grid')
 
 addPolys(b,border='red')


	log = lobster.db('process.logs') 
	log38 = subset(log,LFA==38)
	
	NLics = aggregate(LICENCE_ID~SYEAR+GRID_NUM,data=log38,FUN=function(x) length(unique(x)))
	NLicsT = aggregate(LICENCE_ID~SYEAR,data=log38,FUN=function(x) length(unique(x)))
	...
	#Proportion of licence holders with valid logs reporting fishing by grid in LFA 38
	PropLicsT[order(PropLicsT,decreasing=T)]

#Total landings
	
	L = aggregate(WEIGHT_KG~SYEAR+GRID_NUM,data=log38,FUN=sum)
	LT = aggregate(WEIGHT_KG~SYEAR,data=log38,FUN=sum)
