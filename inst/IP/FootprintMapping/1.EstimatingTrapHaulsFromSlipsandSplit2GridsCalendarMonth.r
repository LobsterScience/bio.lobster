...
a$Month = lubridate::month(a$DATE_FISHED)
...
for(i in 1:length(sL)){
  ...
  pTH = aggregate(NUM_OF_TRAPS~GRID_NUM+Month+LFA+SYEAR,data=tmp,FUN=sum)
...
saveRDS(partEffort,'DiscretizedData/TrapHaulsWithinGridandMonth.rds')
