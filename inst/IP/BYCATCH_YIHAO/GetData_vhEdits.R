...
setwd("C:/Users/HowseVJ/OneDrive - DFO-MPO/Bycatch for Yihao")
...
#gr41 = subset(gr41,GRID_NO %in% u4 & GRID_NO %ni% c(860,1199))   #############SHOULD THIS BE ID ? THERE IS no GRID_NO in gr41 
...
saveRDS(gg,file='proportionCodSurvi.rds')
...
### LOAD dim.list so FisheryFootprint works
...
b$LFA=rep(c(33,34,35,36,38),each=21)  ##Adjust when years change
b$time <- NULL
...
#saveRDS(a,'atSeaDataYihao_wts.rds')  ## at this point a is just unfiltered / un-summarized process logs? Assuming we aren't sending that.
