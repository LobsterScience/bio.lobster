##Design based estimates of Cod Catch
require(bio.lobster)
...
aO = bycatch.db('ISDB.reshape')
 ...
 ao = aggregate(CodWt~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=mean)
 aCs = aggregate(CodWt~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CodWt'),c('CodWtvar'))
        aCC = aggregate(UID~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=function(x) length(unique(x)))
        ao = merge(ao, aCC)
        aCo = merge(ao, aCs)
        aCo$mean =aCo$CodWt
        aCo$var =aCo$CodWtvar
        aCo$n = aCo$UID
 
 ap = aggregate(CodWt~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=mean)
 aCs = aggregate(CodWt~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CodWt'),c('CodWtvar'))
        aCC = aggregate(UID~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=function(x) length(unique(x)))
        ap = merge(ap, aCC)
        aCp = merge(ap, aCs)
        aCp$mean =aCp$CodWt
        aCp$var =aCp$CodWtvar
        aCp$n = aCp$UID
...
#4missing Strata with minimal effort
	...
	gs = as.data.frame(do.call(rbind,out))
	 names(gs) = c('LFA','TotCodWt','TotCodWtL','TotCodWtU')
...
aS = bycatch.db('GCIFA')
 ...
 ap = aggregate(CodWt~TRIP+LFA+Cluster+Period+SYEAR,data=aS,FUN=mean)
 aCs = aggregate(CodWt~TRIP+LFA+Cluster+Period+SYEAR,data=aS,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CodWt'),c('CodWtvar'))
 ...
 aTs = rm.from.list(aTs)
  for(i in 1:length(aTs)){
	...
	gs = as.data.frame(do.call(rbind,out))
	names(gs)= c('LFA','YR','TotCod','TotCodL','TotCodW')
	gs = toNums(gs,2:5)
	gs[,3:5] = gs[,3:5]/1000
