##Design based estimates of Cusk Catch
require(bio.lobster)
...
aO = bycatch.db('ISDB.reshape')
 ...
 ao = aggregate(CuskWt~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=mean)
 aCs = aggregate(CuskWt~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CuskWt'),c('CuskWtvar'))
        aCC = aggregate(UID~TRIP+LFA+Cluster+GridGroup+Period,data=aO,FUN=function(x) length(unique(x)))
        ao = merge(ao, aCC)
        aCo = merge(ao, aCs)
        aCo$mean =aCo$CuskWt
        aCo$var =aCo$CuskWtvar
        aCo$n = aCo$UID
 
 ap = aggregate(CuskWt~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=mean)
 aCs = aggregate(CuskWt~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CuskWt'),c('CuskWtvar'))
        aCC = aggregate(UID~TRIP+LFA+Cluster+GridGroup+Period,data=aS,FUN=function(x) length(unique(x)))
        ap = merge(ap, aCC)
        aCp = merge(ap, aCs)
        aCp$mean =aCp$CuskWt
        aCp$var =aCp$CuskWtvar
        aCp$n = aCp$UID
...
#4missing Strata with minimal effort
     ...
     uy = read.csv('data/GridGroupsLocation.csv')
	  ...
	  for(i in 1:length(re)){
	  		...
	  		mum = sum(k$mean * k$BTTH)/sum(k$BTTH)
		  	...
		  	for(j in 2:(iter)){
	  				...
	  				mum = c(mum,sum(k$mean * k$BTTH)/sum(k$BTTH))
	  				v = c(v,sqrt(sum((kw)^2*(k$var/k$n))))
	  				ql =c(ql,qnorm(.025,mu[j],v[j]))
	  				qu = c(qu,qnorm(.975,mu[j],v[j]))
		  }
		out[[i]] = c(re[i],mean(mu),mean(ql),mean(qu),mean(mum))
		}
	  
	gs = as.data.frame(do.call(rbind,out))
	 names(gs) = c('LFA','TotCuskWt','TotCuskWtL','TotCuskWtU')
...
aS = bycatch.db('GCIFA')
 ...
 ap = aggregate(CuskWt~TRIP+LFA+Cluster+Period+SYEAR,data=aS,FUN=mean)
 aCs = aggregate(CuskWt~TRIP+LFA+Cluster+Period+SYEAR,data=aS,FUN=var,na.rm=T)
        aCs = rename.df(aCs,c('CuskWt'),c('CuskWtvar'))
 ...
 aTs = rm.from.list(aTs)
  for(i in 1:length(aTs)){
	...
	gs = as.data.frame(do.call(rbind,out))
	names(gs)= c('LFA','YR','TotCusk','TotCuskL','TotCuskW')
	gs = toNums(gs,2:5)
	gs[,3:5] = gs[,3:5]/1000
