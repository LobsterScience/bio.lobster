...
i = which(survey$Legal>50 & survey$SOURCE =='AT_SEA_SAMPLES')
survey = survey[-i,]
i = which(survey$Legal<0)
survey = survey[-i,]
...
ns_coast <- suppressWarnings(suppressMessages(
  st_crop(ns_coast,
          c(xmin = -72, ymin = 41, xmax = -56.5, ymax = 47.5))))
...
#rerun july10 2023
survey$pa = ifelse(survey$Legal>0,1,0)
ss = as_tibble(survey)
fit = sdmTMB(pa~
               s(lZ,k=4)+s(BT,k=3)+Q,
             data=ss,
...
#grids
rLL = readRDS(file.path( project.datadirectory("bio.lobster"), "data","maps","GridPolysSF.rds"))
rLL = st_as_sf(rLL)
st_crs(rLL) <- 4326
rLL = st_transform(rLL,32620) 
st_geometry(rLL) <- st_geometry(st_as_sf(rLL$geometry/1000)) 
st_crs(rLL) <- 32620

ff = st_join(gsf,rL,join=st_within)
gsf = subset(ff,!is.na(LFA))

ff = st_join(gsf,rLL,join=st_within)
gsf = subset(ff,!is.na(LFA))


saveRDS(list(fit,gsf),'preds_sdmTMBbyQbinoJuly112023.rds')
xx=readRDS('preds_sdmTMBbyQbinoJuly112023.rds')
fit=xx[[1]]
gsf=xx[[2]]
...
ggplot(subset(gsf,!is.na(LFA)),aes((preds)))+
  geom_histogram(aes(y=..density..),position='identity')+
  facet_wrap(~LFA)+
  geom_vline(xintercept=0)
  savePlot('DistributionOfPredictedDensityLegalLobJuly2023.png') 
