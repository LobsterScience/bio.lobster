	...
	for(i in 1:length(yrs)){
	  cpuegrids = lobGridPlot(subset(logsInSeason,LFA%in%p$lfas&SYEAR == yrs[i],c("LFA","GRID_NUM","logCPUE")),FUN=mean,lvls=cpueLevels)	
	...
	potLevels = c (0,60,120,180,240,300,360,420,480)
	yrs = 2011:2016
	for(i in 1:length(yrs)){
	potgrids = lobGridPlot(subset(logsInSeason,LFA%in%p$lfas&SYEAR == yrs[i],c("LFA","GRID_NUM","NUM_OF_TRAPS")),FUN=mean,lvls=potLevels) 
	pdf(file.path(figdir,paste0("FishFootpot", yrs[i],".pdf")))
	LobsterMap('27-33',poly.lst=potgrids)
	SpatialHub::contLegend('bottomright',lvls=potgrids$lvls,Cont.data=potgrids,title="Mean Pots Hauled",inset=0.02,cex=0.8,bg='white')
	dev.off()
	}
