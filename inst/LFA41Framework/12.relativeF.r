...
b = c('stratified.summer.LFA41.restratified.length.all.not.sexed.rdata')
...
#summer survey
		...
		#  
			...
			pdf(file.path( project.figuredirectory("bio.lobster"), "SummerSurveySeasonalFishingPattern.pdf"),8,11)
		...
		La$landings = La$ADJCATCH / 1000
		La = rename.df(La,'fishingYear','yr')
	 png(file=file.path(fn,'SummerSurveyLandings.png'),units='in',width=15,height=12,pointsize=18, res=300,type='cairo')
		plot(La$yr,La$landings,type='l',xlab='Year',ylab='Landings')
		dev.off()
