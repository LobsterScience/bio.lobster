...
gS = aggregate(cbind(WEIGHT_KG,NUM_OF_TRAPS)~LFA+SYEAR,data=subset(a,GRID_NUM %in% gr & LFA ==34),FUN=sum)
names(gS)[3:4] = c('SMBWT','SMBNT')
gA = aggregate(cbind(WEIGHT_KG,NUM_OF_TRAPS)~LFA+SYEAR,data=subset(a, LFA ==34),FUN=sum)
