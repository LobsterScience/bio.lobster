...
figdir = file.path(project.datadirectory("bio.lobster","requests","ENS LFA 27 Grid Breakdown"))
...
#Determine what ports (by name) report in what grids 
l =  lobster.db('process.logs')
l=subset(l,SYEAR==assessment.year & LFA=="27" )

cc=read.csv("Community Codes.csv")

l <- l %>%
    left_join(cc %>% dplyr::select(COMMUNITY_CODE, COMMUNITY_NAME), by = "COMMUNITY_CODE") %>%
    mutate(COMMUNITY_CODE = COMMUNITY_NAME) %>%
    dplyr::select(-COMMUNITY_NAME)

b <- l %>%
    group_by(GRID_NUM) %>%
    summarise(community_codes = paste(sort(unique(COMMUNITY_CODE)), collapse = ", "))

write.csv(b, file = "Ports by Grid.csv", row.names = FALSE)


# Fishery footprint-a =  lobster.db('process.logs')
#------------------------------------------------------------
...
a = subset(a,SYEAR>=2010 & SYEAR<=assessment.year) #subsetting data to 2010-2024
...
g27p = subset(g27p, LFA=="27" )

#run to here


g27p$LandingsT = g27p$Landings/1000
g27p$Landingspkm2 = as.numeric(g27p$LandingsT)/as.numeric(g27p$area)
g27p$TrapHaulspkm2 = as.numeric(g27p$TrapHauls)/as.numeric(g27p$area)
g27p$Licencespkm2 = as.numeric(g27p$NLics)/as.numeric(g27p$area)


g27p = subset(g27p, select = -c(V2, grid) )

names(g27p)=c("LFA", "Reporting_Grid", "Area", "Year", "Trap Hauls", "Landings (kg)", "Trips", "Licenses Reported", "geometry","CPUE (kg/trap)", "Landings (mt)", "Landings / km2" , "Trap Hauls / km2", "Licenses / km2")

g27p <- g27p %>% #order by grid then year
    arrange(REPORTING_GRID, FISHING_YEAR)

saveRDS(g27p,'lobster.pruned.grid.data.rds')

#For summary to share, only include the first instance of grid / year combo
#All information is contained, just not every small polygon

summary.lfa27 <- g27p %>%
      distinct(Reporting_Grid, Year, .keep_all = TRUE)
summary.lfa27 = subset(summary.lfa27, select = -c(geometry))
