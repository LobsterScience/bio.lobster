...
png(filename=file.path(figdir, "MapLFA29.png") ,width=6.5, height=6.5, units = "in", res = 800)
...
# First, calculate median CPUE by SYEAR and LFA = "29"
df_lfa30 <- a %>%
    filter(LFA == "30", !GRID_NUM %in% c(345)) %>%
    group_by(SYEAR) %>%
    summarize(median_cpue = median(CPUE, na.rm = TRUE), .groups = "drop")
...
# Create the plot with both lines
png(filename=file.path(figdir, "LFA30.Adjacent.and.not.cpue.png"),width=6, height=4, units = "in", res = 800)
ggplot() +    # Line for all LFA 30
    geom_line(data = df_lfa30, aes(x = SYEAR, y = median_cpue, color = "LFA 30 (no Grid 345)"), size = 1) +    
    # Line for Grid 344
    geom_line(data = df_345, aes(x = SYEAR, y = median_cpue, color = "Grid 345"), size = 1) + 
    # Points for all LFA 30
    geom_point(data = df_lfa30, aes(x = SYEAR, y = median_cpue, color = "LFA 30 (no Grid 345)")) +    
    # Points for grid 345
    geom_point(data = df_345, aes(x = SYEAR, y = median_cpue, color = "Grid 345")) +   
    # Labels and title
    labs(title = "CPUE by YEAR",
         x = "Year",
         y = "CPUE (kg/trap haul)",
    ...
    scale_y_continuous(limits = c(0, NA)) +     # Setting the y-axis to start from 0
    # Custom color scale
    scale_color_manual(values = c("LFA 30 (no Grid 345)" = "blue", "Grid 345" = "red")) +
    # Minimal theme for a clean plot
    #theme_minimal() +
    # Move the legend to the bottom-right corner
    theme(legend.position = c(0.85, 0.15),
