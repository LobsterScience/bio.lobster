...
#require(bio.utilities)
 ...
 LobsterMap('custom',ylim=c(45.9,46.3), xlim=c(-61.25,-60.2),labels= c('grid', 'lfa')) 
 addPolys(b,border='red')


	log = lobster.db('process.logs') 
	log27 = subset(log,LFA==27)
	
	#last five years
	myr=max(log27$SYEAR)
	minyr=myr-4
	log27=log27[log27$SYEAR %in% c(minyr:myr),]
	
	NLics = aggregate(LICENCE_ID~SYEAR+GRID_NUM,data=log27,FUN=function(x) length(unique(x)))
	NLicsT = aggregate(LICENCE_ID~SYEAR,data=log27,FUN=function(x) length(unique(x)))
	...
	#Proportion of licence holders with valid logs reporting fishing by grid in LFA 27
	PropLicsT[order(PropLicsT,decreasing=T)]

#Total landings
	
	L = aggregate(WEIGHT_KG~SYEAR+GRID_NUM,data=log27,FUN=sum)
	LT = aggregate(WEIGHT_KG~SYEAR,data=log27,FUN=sum)
	...
	PropL = na.zero(round(L / LT,5))

	PropLT = apply(PropL,2,mean,na.rm=T)
	#Proportion of landings from logs reporting fishing by grid in LFA 276
