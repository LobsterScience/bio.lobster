...
#ref points
    ub = median(subset(aout,yr %in% 2001:2015,select=w.Yst)[,1])/1000 * 0.4
   ...
   write.csv(aout,file=file.path('~','tmp','NEFSC.spring.restratified.commercial.csv'))
...
ub = median(subset(aout,yr %in% 2001:2015,select=w.Yst)[,1])/1000 * 0.4
  ...
  write.csv(aout,file=file.path('~','tmp','NEFSC.fall.restratified.commercial.csv'))
     rm(aout)
...
#refpoints
      ub = median(subset(aout,yr %in% 2000:2015,select=w.Yst)[,1])/1000 * 0.4
              llb = aout$w.Yst[which(aout$w.Yst>0)]
              llb = median(sort(llb)[1:5])/1000
 ...
 #total
  ...
  write.csv(aout,file=file.path('~/tmp','DFO.restratified.commercial.csv'))
...
#DFO Georges
    ...
    p$define.by.polygons = F
    p$lobster.subunits=F
    p$years.to.estimate = c(2007:2019)
    p$length.based = T
    p$by.sex = T
    p$sex = c(1,2)
    p$size.class = c(83,300)
...
#ref points
    ...
    write.csv(aout,file=file.path('~/tmp','DFO.Georges.commercial.csv'))
