...
CompSurvey2<-LobsterSurveyProcess(lfa=c('L35','L36'),yrs=2019,mths=c("Sep"),bin.size=1)
...
outBB = gamlss(cbind(NEST,BALL)~cs(Length,df=3),nu.formula=~cs(Length,df=3),data=dat,family=BB())
