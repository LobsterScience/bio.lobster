...
# ToDo- Need to add an exception for John  Rockett in 2023 who did 46mm vs 47mm
...
if(length(probs$date)>0) {View(probs)}else {print("Same # of 'Test' & 'Standard' vents for each date for each fisher")}

rm(test44, test47, test46, probs, data.check,data.check.rock, data.check2, nulls, test)
}
  ...
  size.class.histogram=function(plot.t=titl){#plot the occurrence of each size class by config          
    ...
    print(
      ...
      ggplot(data=bar.p, aes(x=variable, weight=value, fill=Vent.Configuration))+
        ...
        theme(panel.background = element_blank()) +
        scale_y_continuous(expand = c(0,0))  +
        #Following 3 lines add text under the legend
        labs(tag = paste("Trap Hauls: ", trap.tot)) +
        coord_cartesian(clip = "off") +
        theme(plot.tag.position = c(.865, .445))+
...
for (y in yrs){
  ...
  dir.create( fishdir, recursive = TRUE, showWarnings = FALSE )
        ...
        #Save Figures
        #config.comparison()
        png(filename=paste0(fishdir,"/",f, ".", y, ".config.comparison.png"),width = 160, height = 150, units='mm', res = 300)
