...
LobsterMap('BoF')
 ...
 #with(surveyCatch,identify(SET_LONG,SET_LAT))
...
FundyStns = rbind(FundyStns,x)
write.csv( FundyStns ,file.path(project.datadirectory('bio.lobster'),"data","survey","FundyStations.csv"),row.names=F)

LobsterMap('35-36')
 with(FundyStns,points(X,Y,col='red',pch=16))


sbf=subset(AllSurveyDataR0,LFA%in%c(35,36)&survey=="DFOsummer"&LobCatch>0)
with(sbf,points(X,Y,bg='green',pch=21))

 with(FundyStns,points(X,Y,bg='red',pch=21))
 #with(sbf,identify(X,Y))

y=sbf[c(9,  18,  25,  28,  33,  35,  43,  50,  51,  54,  58,  80,  84,  89,  90,  97, 104, 113, 117, 120, 128, 129, 131),c('X','Y')]
y$TYPE='GF'

FundyStns = rbind(FundyStns,y)
...
xbf=subset(AllSurveyDataR0,LFA%in%c(35,36)&survey=="Scallop"&LobCatch>1)
with(xbf,points(X,Y,bg='green',pch=21))

 with(FundyStns,points(X,Y,bg='red',pch=21))
 
 #with(xbf,identify(X,Y))
z=xbf[c(18, 26, 38, 49, 50, 78, 86, 89, 91),c('X','Y')]
z$TYPE='SC'

FundyStns = rbind(FundyStns,z)
...
w=g[c(60939, 65831,  66027,  75595,  78571, 102827, 104915, 109447, 125895, 130627, 165759),c('X','Y')]
w$TYPE='NEW'

FundyStns = rbind(FundyStns,w)

56271  64583  70743  82103  90387 106443 116223 124187 127075 127079 162467 173727


w=g[c(56271,  64583,  70743,  82103,  90387, 106443, 116223, 124187, 127075, 127079, 162467, 173727),c('X','Y')]
w$TYPE='NEW'
...
w=g[c(42127,  57799,  82211, 154415),c('X','Y')]
w$TYPE='NEW'
